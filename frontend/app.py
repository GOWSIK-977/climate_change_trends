...
with st.sidebar:
    ...
    data_source_info = APIClient.get_data_source()
    
    st.warning(
        f"""
        📊 **Data Source:** {data_source_info.get('data_source', 'Unknown')}
        
        📁 **File:** {data_source_info.get('data_source', 'Unknown')}
        
        📈 **Total Records:** {data_source_info.get('total_records', 'N/A')}
        
        📅 **Year Range:** {data_source_info.get('year_range', 'N/A')}
        """
    )
...
if page == "🏠 Dashboard":
    ...
    if climate_data:
        ...
        with col4:
            ...
            <div class="metric-card">
                <h3>🎯 R² Score</h3>
                <h2 style="color: #51cf66;">{APIClient.get_trend_analysis('temperature_2m').get('r_squared', 0)}</h2>
